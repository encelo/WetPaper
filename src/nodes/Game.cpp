...
void Game::onTick(float deltaTime)
{
		...
		for (unsigned int i = 0; i < len; i++)
		{
			...
			for (unsigned int j = i + 1; j < len; j++)
			{
				...
				if (bA->colliderKind() == ColliderKind::CIRCLE && bB->colliderKind() == ColliderKind::CIRCLE)
					...
					LOGW_X("Unhandled: %s vs %s", Body::colliderKindToName(bA->colliderKind()), Body::colliderKindToName(bB->colliderKind()));
			}
		}
	}
	...
	redPos.x += (blueBar_->width() * (1.0f - playerA_->stamina())) * 0.5f;
...
void Game::drawGui()
{
	...
	ImGui::Text("Collisions: %d", Body::Collisions.size());
...
void Game::loadScene()
{
	...
	obstacle1_->colliderHalfSize_ = nc::Vector2f(4096.0f, 32.0f);
...
void Game::spawnBubble()
{
	...
	const int variant = nc::random().integer(0, Cfg::Textures::NumBubbleVariants);
